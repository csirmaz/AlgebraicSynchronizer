(* Algebraic Synchronizer by Elod P Csirmaz 2000
   ...
   For a version intended to work with unsion
   please see Reconciler-Module.ml
   (unison is Copyright 1999-2012, Benjamin C. Pierce,
   please see <http://www.cis.upenn.edu/~bcpierce/unison/>).
*)
(* This file can be used in the interactive ocaml by typing
   # #use "rec_str.ml";;
   This allows the user to try out implemented functions.
   Paths are represented by strings:
     (path) := "-"^(string)
   where every character in the string denotes a path entry.
   Empty string is denoted by "-".
   Fingerprints are arbitrary strings.

   After loading this file one may say:
   # reconciliator
     [ (Remove "-ab"); (Remove "-abc") ]
     [ (Edit ("-abc",File "x")) ]
     ;;
   for the second example in the paper.
*)
(*
...
(**************************************)
  type command
    = Remove of (*Path.path*) string
    | Edit   of (*Path.path*) string * ctype
    | Create of (*Path.path*) string * ctype
  and ctype
    = File of (*Fingerprints.file_fp*) string
    | Dir  of (*Fingerprints.dir_fp*) string
...
(* These commands have been rewritten for the string-representation *)

(* current model of paths: path = "-"^(string) where every character is
     an entry. emptypath = "-". *)
...
let root = fun p -> 
  if(p = "-") then "-" else ("-"^(String.sub p 1 1))
...
let tail = fun p ->
  if(String.length p <= 2) then "-" else
